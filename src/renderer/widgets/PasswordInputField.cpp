...
void CPasswordInputField::setZindex(int zindex) {
    m_iZindex = zindex;
}
    ...
    try {
        ...
        if (props.contains("zindex")) {
            try {
                ...
                Debug::log(LOG, "PasswordInputField configured with zindex: {}", m_iZindex);
            } catch (const std::exception& e) {
                Debug::log(WARN, "Failed to parse zindex for input-field: {}, defaulting to 30", e.what());
                m_iZindex = 30;
            }
        } else {
            m_iZindex = 30;
